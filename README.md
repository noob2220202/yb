...
**"수동 계산기"** 섹션은 스캐너/DB와 완전히 별개로 동작합니다 — 마켓별로 이미
행과 열이 고정된 표(승무패, 승패(무승부 없음), 유럽식 핸디캡(3-way), 아시안
핸디캡(쿼터 라인 포함), 오버언더, 양팀득점, 정확한 스코어)가 미리 그려져 있고,
북메이커 이름과 배당만 빈칸에 채워 넣으면 됩니다 (북메이커 행은 "+ 북메이커 추가"로
얼마든지 늘릴 수 있음). 마켓/선택지를 매번 드롭다운으로 고르던 이전 방식 대신
스프레드시트처럼 빈칸만 채우는 구조로 바꿨고, 표는 모바일 폭에서도 가로 스크롤이
되도록 만들어 작은 화면에서도 그대로 씁니다. 백엔드의 `POST /calculator/scan`이 입력을
...
> 참고: 백엔드 스키마는 여전히 `ScanLegIn.group`으로 서로 다른 마켓(예: 승무패 +
> 오버언더)을 억지로 한 그룹에 묶어 배당 합을 계산하는 기능을 지원하지만, 같은
> 경기의 다른 마켓끼리는 통계적으로 독립이 아니라서(이기는 팀과 총 득점은 서로
> 영향을 줌) 이렇게 묶은 결과는 절대 `verified: true`가 될 수 없고 실질적으로
> 오해를 사기 쉬워 현재 UI에서는 이 필드를 노출하지 않습니다. "서로 다른 경기의
> 선택지를 자유롭게 조합"하고 싶다면 아래 **"시스템 베팅"** 도구를 쓰세요 — 그게
> 수학적으로 실제 의미 있는 조합 방식입니다.
...
수동 계산기 아래 별도로 있는 **"시스템 베팅"** 도구는 서로 독립적인 선택지(보통
서로 다른 경기의 결과)를 조합해 "N개 중 M개 이상 적중"하면 수익이 나는 시스템
베팅(Trixie/Yankee/Canadian/Heinz/Super Heinz/Goliath 같은 이름 붙은 상품들이
전부 이 "System M/N"의 특수 케이스)을 계산합니다. `POST /calculator/system-bet`
백엔드(`app/engine/systembet.py`)가 각 선택지 조합(크기 M부터 N까지의 모든
부분집합, 최대 `C(N,M)+...+C(N,N)`개)에 똑같은 총 베팅 금액을 균등 분배하고,
목표 적중률(%)을 만족하는 가장 작은 M을 자동으로 찾아줍니다(포아송-이항분포로
"N개 중 M개 이상 적중할 확률"을 정확히 계산).

**확정 수익 픽과는 완전히 다른 성격**이라는 걸 결과 카드에 항상 명시합니다 — 이건
배당 역수 합이 100% 미만이라 결과와 무관하게 이기는 수학적 보장이 아니라,
기댓값(EV) 기반 계산입니다. 각 선택지의 "내 예상 승률(%)"을 직접 입력하면 그
값으로 기대수익을 계산하고, 비워두면 배당의 역수(1/배당)를 승률로 대신
씁니다 — 다만 **배당에서 그대로 뽑은 확률로 계산하면 어떤 배당을 넣든 기대수익은
항상 정확히 0%에 수렴합니다** (`stake × p × odds = stake × (1/odds) × odds =
stake`이므로 순환 논리가 되어버림). 그래서 진짜 엣지를 보려면 배당과는 독립적인
자신만의 확률 추정치를 넣어야 하고, 넣지 않았을 때는 결과 카드에 "배당의
역수를 그대로 썼다"는 경고가 뜹니다. 또한 **같은 경기의 서로 다른 마켓을
섞으면 안 됩니다** — 승리와 오버언더처럼 같은 경기 안 마켓끼리는 서로 통계적으로
독립이 아니라서 이 계산이 가정하는 독립성이 깨지고 결과가 부정확해집니다.
선택지는 한 번에 최대 16개까지 지원합니다 (그 이상은 조합 수가 기하급수적으로
늘어나 비현실적).
